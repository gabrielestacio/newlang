...
No terminal, com as ferramentas Lex e Yacc instaladas, dentro da pasta ***lang***, rode os comandos:
...
### **for**
A instrução *for* cria um loop que consiste em três expressões separadas por ponto e vírgula, seguidas por um bloco de instruções a serem executadas no loop.
```c
for (int i = 0; i < 7; i++) {
  print(i);
}
...
### **while**
A instrução *while* cria um loop que executa um bloco de instruções específicas desde que a condição de guarda seja verdadeira. A condição é avaliada antes de executar a instrução.
```c
int n = 0;
...
## Condicionais
### **if...else**
A instrução *if* executa um bloco de instruções se uma condição especificada for verdadeira. Se a condição for falsa, o programa pode tanto executar outras instruções nas cláusulas opcionais *else if/else* ou prosseguir sua execução.
```c
...
### **switch**
A instrução *switch* avalia uma expressão, comparando o valor da expressão com uma série de cláusulas *case* e executa um bloco de instruções após a primeira cláusula *case* com um valor correspondente, até que uma instrução *break* seja encontrada. A cláusula *default* de uma instrução *switch* será retornada se nenhum caso corresponder ao valor da expressão.
```c
...
Retorna *true* se a condição for satisfeita ou *false* caso não seja.
